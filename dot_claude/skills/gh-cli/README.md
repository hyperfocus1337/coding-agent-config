...
If a proper gh command reference skill ever appears in a git repo, moving to apm is a two-line addition under `dependencies.apm` (a `git:` entry plus the skill name), mirroring the existing `neondatabase/agent-skills` block, and this local copy can then be deleted.
