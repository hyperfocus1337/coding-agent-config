...
The three skills carry `disable-model-invocation: true`, so this skill is the only entry point that is auto-selected. Each stays invocable by exact name.
...
Invoke the skill of that row by name and pass the resolved id, the row, and the target project. Do not repeat its procedure here. Each skill asks the scope itself, with `project`, `local`, and `user`, and preselects the row's `scope`.
