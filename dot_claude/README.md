...
| [`format-org-tables`](hooks/format-org-tables/README.md)               | `PostToolUse` (Write/Edit)       | Re-aligns Org tables in edited `.org` files via `emacs --batch`. Needs `emacs`.                                                                                                          |
...
| [`block-secret-commits`](hooks/block-secret-commits/README.md)         | `PreToolUse` (Bash)              | Blocks `git add`/`git commit` on non-gitignored secret files (`.env`, keys, creds), and `git commit` on a secret value in the staged content. Needs `betterleaks` for the content check. |
| [`enforce-cli-tools`](hooks/enforce-cli-tools/README.md)               | `PreToolUse` (Bash)              | Blocks banned CLI tools in command position and names the replacement (`npm`/`yarn` → `pnpm`). Table-driven.                                                                             |

Every hook caps its runtime with a timeout in `settings.json` and self-disables cleanly when its tools are missing. The advisory formatting hooks swallow failures and exit 0, so a hiccup never blocks a tool call; the lint, type-check, and secret-commit hooks exit 2 to surface errors or a block back to Claude. Shell hooks parse the payload with [`jq`](https://jqlang.github.io/jq/), so `jq` must be on `$PATH` (`brew install jq` on macOS); the formatter needs `prettier` on `$PATH`. See each hook's README for its specific dependencies and install commands.
